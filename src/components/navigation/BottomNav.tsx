...
export default function BottomNav() {
  ...
  const navItems = [
    {
      ...
      icon: Globe,
      href: "/mapa",
      // Define Mundo como ativo se estiver no mapa, trilha ou arena
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-50 bg-white border-t-[3px] border-[#e5e5e5] px-6 py-2 pb-[calc(env(safe-area-inset-bottom)+0.5rem)] shadow-[0_-4px_10px_-2px_rgba(0,0,0,0.05)]">
          ...
          return (
            <Link
              ...
              className={cn(
                ...
                active ? "text-[var(--color-brand)]" : "text-gray-400 hover:text-gray-600"
              )}
            >
              <div
                className={cn(
              ...
              <span
                className={cn(
                  ...
                  active ? "text-[var(--color-brand)]" : "text-gray-400"
                )}
              >
                {item.label}
              </span>
